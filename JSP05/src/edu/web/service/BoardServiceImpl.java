...
public class BoardServiceImpl implements BoardService {
	...
	public List<Board> select() {
		System.out.println("borardServiceImpl.select() 메서드 호출");
	...
	public Board select(int bno) {
		...
		int result = boardDao.update(bno); // 글 번호의 조회수 1 증가
	...
	public List<Board> select(int type, String keyword) {
		System.out.println("boardServiceImpl.selet(type=" + type + ", keyword=" + keyword + ") 메서드 호출");
