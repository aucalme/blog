...
public interface BoardDao {
	...
	// insert into boards (title, content, userid) values (?, ?, ?);
	...
	/**
	 * 글 번호(bno)를 argument로 전달받아서 DB board 테이블에서 게시글 1개를 검색하고,
	 * 그 결과를 리턴.
	 * 
	...
	String SQL_SELECT_BY_TITLE_OR_CONTENT = String.format(
			"select * from BOARDS where lower(%s) like ? or lower(%s) like? order by %s desc", 
			TABLE_BOARD, COL_TITLE, COL_CONTENT, COL_BNO);
	
	// 작성자로 검색: select * from BOARDS where lower(USERID) like ? order by BNO desc;
	String SQL_SELECT_BY_USERID = String.format(
			"select * from BOARDS where lower(%s) like ? order by %s desc", 
	 ...
	 * 
	 * @param type 검색타입(1-제목, 2-내용, 3-제목 + 내용, 4-작성자)
	 * @param keyword 검색어.
	 * @return 검색 결과.
	 */
